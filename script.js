...
(function () {
  ...
  if (!form) return;
  var EMAILJS_PUBLIC_KEY = 'MNE8P7NQsy9TcbLk1';
  var EMAILJS_SERVICE_ID = 'service_rj8cq0a';
  var EMAILJS_TEMPLATE_ID = 'template_yku7anb';
  var emailReady = typeof emailjs !== 'undefined';
  ...
  form.addEventListener('submit', function (e) {
    ...
    var senderName = nameInput ? String(nameInput.value || '').trim() : '';
    var messageText = messageInput ? String(messageInput.value || '').trim() : '';
    ...
    emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      name: senderName,
      message: messageText,
      attendance: rsvpInput ? rsvpInput.value : ''
    }).then(function () {
      ...
      form.reset();
      var rsvpBtnsReset = form.querySelectorAll('.rsvp-btn');
      rsvpBtnsReset.forEach(function (b) { b.classList.remove('selected'); });
      if (rsvpInput) rsvpInput.value = '';
    }).catch(function () {
      if (status) {
